...
func GetProxyGroupIDByName(name string) (string, error) {
	for _, proxyGroupId := range GetAllProxyGroupIds() {
		proxyGroup, err := GetProxyGroup(proxyGroupId)
		if err != nil {
			return "", err
		}

		if proxyGroup.Name == name {
			return proxyGroupId, nil
		}
	}
